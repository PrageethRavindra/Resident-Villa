...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] === 'check_driver_availability') {
    ...
    $pickup_location = htmlspecialchars($booking_data['pickup_location'], ENT_QUOTES, 'UTF-8');
    $destination = htmlspecialchars($booking_data['destination'], ENT_QUOTES, 'UTF-8');
    $fare = filter_var($booking_data['fare'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
    $booking_date = validateDate($booking_data['booking_date']) ? $booking_data['booking_date'] : null;
    $pickup_time = validateTime($booking_data['pickup_time']) ? $booking_data['pickup_time'] : null;
    $dropoff_time = validateTime($booking_data['dropoff_time'], 'H:i:s') ? $booking_data['dropoff_time'] : null;
    $email = filter_var($booking_data['email'], FILTER_SANITIZE_EMAIL);
    $fname = htmlspecialchars($booking_data['fname'], ENT_QUOTES, 'UTF-8');

    // Check for valid inputs before proceeding
    if (!$booking_date || !$pickup_time || !$dropoff_time) {
        $response = ['available' => false, 'error' => 'Invalid date or time format'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['action'])) {
    if (!empty($_POST['fname']) && !empty($_POST['email']) && !empty($_POST['pickup']) && !empty($_POST['drop']) && !empty($_POST['VehicleType'])) {
        // Sanitize inputs
        $vehicleType = htmlspecialchars($_POST['VehicleType'], ENT_QUOTES, 'UTF-8');
        $pickup = htmlspecialchars($_POST['pickup'], ENT_QUOTES, 'UTF-8');
        ...
        $pickupDate = validateDate($_POST['PickupDate']) ? $_POST['PickupDate'] : null;
        $pickupTime = validateTime($_POST['pickup_time']) ? $_POST['pickup_time'] : null;
        $drop = htmlspecialchars($_POST['drop'], ENT_QUOTES, 'UTF-8');
